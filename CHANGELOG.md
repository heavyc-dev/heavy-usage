...
- **Release tooling.** `scripts/check-version-sync.js` asserts the version is identical across `package.json`, both manifests, and the top CHANGELOG heading (CI step, plus a `--tag` check at release time); `scripts/bump.js` writes all of them from one command; a tag-triggered `release.yml` verifies the tag matches and publishes the GitHub release. Added `package.json` (no deps) and flipped `claude plugin validate` to hard-fail in CI.
