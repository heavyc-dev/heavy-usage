...
- **`/sweep` — a usage-budgeted, resumable codebase sweep (the loop's killer app).** Bundles a new skill (`skills/sweep/`) plus two read-only subagents (`sweep-scout`, `sweep-verifier`). It runs a repeated review/discovery pass and **scales effort to remaining headroom**: while you have room it fans out scouts in parallel, and as usage approaches wind-down it collapses to a single inline pass — then stops cleanly at the wall with findings committed and a resume note written. Canonical run: `/loop /sweep <mode>` (the `/loop` skill re-invokes; this skill does each iteration; the wind-down hook ends the loop).
  ...
  - **Scope once, run unattended:** the first pass per mode asks a few scope questions (focus paths, severity floor, dimensions…) via one prompt, persists them, and never re-prompts; `/sweep <mode> auto` and scheduled/`/loop` resumes skip the interview entirely so headless runs never hang.
  - **Stale-aware tiering:** subagent rounds burn usage fast and the captured numbers go stale between them — the tier logic treats `stale` as "assume higher" and drops a tier, so it won't blow past the wall (builds directly on the v1.2.0 stale guard).
  - **Deliverables vs bookkeeping:** docs land in `heavy-usage-sweep/` (committed on wind-down); the coverage cursor/dedupe live in `.heavy-usage-sweep/` (gitignored, also auto-added to the target repo's `.gitignore`).
  - **Post-reset resume:** optionally schedules a one-shot cron resume just after `resets_at` to continue the sweep next window.
  - New `scripts/sweep-state.js` (pure Node, no deps) owns the config/index/tier/dedupe engine and reuses `usage-lib.js`; +6 unit tests (52 total).
